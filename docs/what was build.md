...
Smart Invest is a mobile-first investment platform built with **Java 21 + Spring Boot 3.3** on the backend and **React 18 + TypeScript + Vite** on the frontend. The project uses a multi-module Maven architecture, Flyway for database migrations, and JWT (RS256) for authentication.
...
```bash
docker-compose up -d
# PostgreSQL: localhost:5432, user smartadmin / localdev_only
```
