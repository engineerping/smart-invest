...
import com.smartinvest.fund.client.OrderServiceClient;
...
public class PortfolioService {
    ...
    public List<?> invest(UUID portfolioId, UUID userId, InvestFromPortfolioRequest req) {
        ...
        if (isMonthly) {
            ...
            return allocations.stream().map(alloc -> {
                int idx = allocations.indexOf(alloc);
                com.smartinvest.common.dto.PlaceOrderRequest orderReq = new com.smartinvest.common.dto.PlaceOrderRequest(
                    alloc.getFundId(),
                    "ONE_TIME",
                    amounts[idx],
                    req.startDate(),
                ...
                return orderServiceClient.placeOrder(userId, orderReq);
            }).toList();
        }
    }
