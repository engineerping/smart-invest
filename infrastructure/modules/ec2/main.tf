...
# =============================================================================
# =============================================================================
# Terraform 语法速查：data 和 resource 的语法含义
# =============================================================================
# data "<数据源类型>" "<本地名称>" { ... }
#   - "数据源类型"（如 aws_ami）：由 Provider 定义，告诉 Terraform「我要查询什么」。
#     命名规范：<provider>_<资源名>
#   - "本地名称"（如 al2023）：自定义名字，通过 data.aws_ami.al2023.id 引用查询结果
#   - data 是只读的，不创建资源，只查询已有信息
#
# resource vs data 的区别：
#   - resource = 创建/管理资源（CRUD 的 Create/Update/Delete）→ terraform destroy 会删除
#   - data    = 查询已有资源（CRUD 的 Read）                   → terraform destroy 不涉及（没有创建过）
# =============================================================================
data "aws_ami" "al2023" {
