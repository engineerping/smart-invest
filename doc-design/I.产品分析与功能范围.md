...
│  │                    │          │                  │                 │
│  │  POST /api/portfolio          │ @Scheduled 定时   │                 │
│  │  自建组合模板管理   │          │ 触发结算/月定投   │                 │
│  │  按比例批量下单    │          │                  │                 │
│  └──────┬──────┬──────┘          └────┬──────┬──────┘                 │
│         │      │                      │      │                        │
...
| 模块 | 依赖 | 说明 |
|------|------|------|
| `module-portfolio` | `module-fund`、`module-order`、`module-plan` | 编排者：校验基金存在、批量下单或创建定投 |
| `module-order` | `module-holding` | 结算完成后调用 `HoldingService.applySettlement()` 更新持仓 |
| `module-holding` | `module-fund` | 查询基金 NAV 计算市值 |
| `module-plan` | `module-fund` | 查询基金信息 |
| `module-scheduler` | `module-order`、`module-plan` | 定时触发结算和月定投执行 |

**关键设计原则：依赖方向单向，无循环。**  
`module-order` 不知道 `module-portfolio` 的存在——`orders` 表仅通过 `portfolio_id` 外键记录关联，业务编排完全由上层的 `module-portfolio` 负责。
